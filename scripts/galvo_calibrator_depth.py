...
class ManualGalvoCalibrationNode:
    ...
    def __init__(self):
        ...
        # 绿色检测参数
        self.green_lower = np.array([32, 40, 45])
    ...
    def detect_green_circles(self, image):
        ...
        self._frame_index += 1
        new_pool = {}
        max_dist_track = 10  # 帧间匹配距离像素
        min_hits_to_show = 3  # 连续命中次数阈值（2~3更稳）
        ...
        for c in picked:
            best_id, best_d = None, max_dist_track
            for tid, t in self._stable_pool.items():
                if tid in used_ids:  # 避免一个ID被多个检测匹配
                    continue
                d = np.hypot(c['center'][0] - t['center'][0], c['center'][1] - t['center'][1])
                if d < best_d:
                    best_id, best_d = tid, d
            ...
            if best_id in self._stable_pool:
                ...
                nx = int(round(0.6 * px + 0.4 * c['center'][0]))
                ny = int(round(0.6 * py + 0.4 * c['center'][1]))
                prev_radius = self._stable_pool[best_id]['radius']
            else:
                nx, ny = c['center']
                prev_radius = c['radius']

            new_pool[best_id] = {
                'center': (nx, ny),
                'radius': int(round(0.5 * prev_radius + 0.5 * c['radius'])),
                'area': c['area'],
                'circularity': c['circularity'],
                'fill': c['fill'],
                'hits': hits,
