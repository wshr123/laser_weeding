...
// *** 更改 #1: 修正了类名的使用 ***
// 即使文件名是 XY2_100_multi.h，类名仍然是 XY2_100

#include <XY2_100_multi.h>
using XY2Driver = XY2_100; // 修正：类名是 XY2_100
...
// ===== 激光控制引脚 =====
const int LASER_PINS[GALVO_COUNT] = {9, 10};  // 激光TTL控制 (开/关)，每个振镜对应一个激光
...
// 激光状态
bool laser_enabled[GALVO_COUNT] = {false, false};      // 激光开关状态
...
void setup() {
  ...
  Serial.println("     Laser Weeding System v1.1");
  Serial.println("=====================================");
  ...
  // *** 更改 #2: 为每个振镜实例提供具体的硬件配置 ***
  // 您必须根据您的实际硬件接线修改下面的引脚号、FTM模块和GPIO端口！
  // 这是一个基于Teensy 3.2的典型配置示例。

  // 配置第一个振镜 (galvo_primary)
  Serial.println("Configuring Primary Galvo (G1)...");
  // 使用 FTM2 定时器, 数据端口为 PORTD
  // 引脚顺序: D0, D1, D2, D3, D4, D5, D6, D7
  // 示例引脚: 2, 14, 7, 8, 6, 20, 21, 5
  galvo_primary.begin(2, &GPIOD_PDOR, 2, 14, 7, 8, 6, 20, 21, 5);
  delay(100);

  // 配置第二个振镜 (galvo_secondary)
  Serial.println("Configuring Secondary Galvo (G2)...");
  // 使用 FTM1 定时器, 数据端口为 PORTC
  // 引脚顺序: D0, D1, D2, D3, D4, D5, D6, D7
  // 示例引脚: 15, 22, 23, 9, 10, 13, 11, 12
  galvo_secondary.begin(1, &GPIOC_PDOR, 15, 22, 23, 9, 10, 13, 11, 12);
  delay(100);
  ...
  Serial.println("  LASER:ON   - Turn active galvo laser on");
  Serial.println("  LASER:OFF  - Turn active galvo laser off");
  Serial.println("  LASER:1/0  - Alternate laser control (ON/OFF)");
  Serial.println("  LASERn:ON  - Turn specific galvo laser on");
  Serial.println("  LASERn:OFF - Turn specific galvo laser off");
...
void loop() {
  ...
  for (uint8_t i = 0; i < GALVO_COUNT; i++) {
    if (laser_enabled[i] && (millis() - laser_start_time[i] > MAX_LASER_ON_TIME)) {
      Serial.println("WARNING: Laser " + String(i + 1) + " timeout - auto shutdown");
      setLaser(i, false);
    }
  }
...
void parseCommand(String cmd) {
  ...
  // 激光控制（支持 LASER:, LASER1:, LASER2:, ...）
  else if (cmd.startsWith("LASER")) {
    int colonIndex = cmd.indexOf(':');
    if (colonIndex > 0) {
      uint8_t target_laser = active_galvo;
      // 检查是否指定了激光索引（如 LASER1: 或 LASER2:）
      if (colonIndex > 5) {
        int idx = cmd.substring(5, colonIndex).toInt() - 1;
        if (idx >= 0 && idx < GALVO_COUNT) {
          target_laser = static_cast<uint8_t>(idx);
        }
      }
      ...
      if (action == "ON" || action == "1") {
        if (laser_mode == LASER_MODE_POINT) {
          setLaser(target_laser, true);
          sendResponse("OK:LASER" + String(target_laser + 1) + ":ON");
        } else {
          int16_t cx = requested_target_x[target_laser];
          int16_t cy = requested_target_y[target_laser];
          drawSpiral(target_laser, cx, cy, spiral_max_radius, spiral_spacing, spiral_angle_step);
          sendResponse("OK:LASER" + String(target_laser + 1) + ":SPIRAL");
        }
      } else if (action == "OFF" || action == "0") {
        setLaser(target_laser, false);
        sendResponse("OK:LASER" + String(target_laser + 1) + ":OFF");
      } else {
        sendResponse("ERROR:INVALID LASER COMMAND");
      }
    } else {
      sendResponse("ERROR:INVALID LASER FORMAT");
    }
    return;
  }
  // 激光模式切换
...
void setLaser(uint8_t galvo_index, bool on) {
  ...
  laser_enabled[galvo_index] = on;
  digitalWrite(LASER_PINS[galvo_index], on ? HIGH : LOW);
  
  if (on) {
    laser_start_time[galvo_index] = millis();
  }
}

void setLaser(bool on) {
  setLaser(active_galvo, on);
}
...
void sendStatus() {
  ...
  for (uint8_t i = 0; i < GALVO_COUNT; i++) {
    ...
    status += ",LASER=" + String(laser_enabled[i] ? "ON" : "OFF") + ")";
  }
